...
interpNum = 2
asCSV = True
cX, cY, cZ = 93.836,137.303,161.627
pX, pY, pZ = 93.868,137.058,163.120
lX, lY, lZ = 94.247,136.164,160.845


username = "TestUser@peaks"
password = ""
# Data return choices
...
userstring += pwb.getBodyA(pdb, interpNum, asCSV, username, password, cX, cY, cZ, lX, lY, lZ, pX, pY, pZ, width, gran,
                           interpMethod, Fos, Fcs, D1, D2, D3, D4, D5, D6, D7, D8, D9)
if access:
    ...
    if havepdb and haveed:
        ...
        if D1 or D2 or D3 or D3 or D4:
            runNo += 1
            print('<p>' + str(runNo) + '/' + str(totalRuns) + ' Calculating peaks...(approx ' + str(
                peaksTime) + ' seconds)...')
            sys.stdout.flush()  # update the user interface
            start = time.time()
            data = Maxima.runCppModule(pdb, interpNum, Fos, Fcs, cX, cY, cZ, lX, lY, lZ, pX, pY, pZ, width, gran, D1,
                                       D2, D3, D4, False, False, False, False, False)
        ...
        if D5 or D6:
            ...
            data = Maxima.runCppModule(pdb, interpNum, Fos, Fcs, cX, cY, cZ, lX, lY, lZ, pX, pY, pZ, width, gran, False,
                                       False, False, False, D5, D6, False, False, False)
        ...
        if D7 or D8:
            ...
            start = time.time()
            data = Maxima.runCppModule(pdb, interpNum, Fos, Fcs, cX, cY, cZ, lX, lY, lZ, pX, pY, pZ, width, gran, False,
                                       False, False, False, False, False, D7, D8, False)
        ...
        if D10 or D11:
            runNo += 1
            print('<p>' + str(runNo) + '/' + str(totalRuns) + ' Creating synthetic density and resampling atoms...(approx 30 minutes !!!)...')
            sys.stdout.flush()  # update the user interface
            start = time.time()
            data = Maxima.runCppModuleSamples(pdb)
            # print(data)
            userstring += pwb.getBodyRun2(pdb, data, False, False, D10, D11,synthetic=True)
            end = time.time()
            ts = getTimeDiff(start, end)
            print('completed in')
            print(ts)
...
cgistring += pwb.getBodyA(pdb, interpNum, asCSV, username, password, cX, cY, cZ, lX, lY, lZ, pX, pY, pZ, width, gran,
                          interpMethod, Fos, Fcs, D1, D2, D3, D4, D5, D6, D7, D8, D9)
