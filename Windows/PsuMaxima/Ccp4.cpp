...
void Ccp4::makePeaks()
{
    ...
    vector<pair<string, float>> tmpmatrix;
    for (map<string,float>::iterator iter = _matrixMap.begin(); iter != _matrixMap.end(); ++iter)
    {
        ...
        tmpmatrix.push_back(std::pair<string, float>(coords, density));
    }    
    //2. sort it on values
    ...
    {
        string coords = tmpmatrix[i].first;
        float density = tmpmatrix[i].second;
        cout << coords << "," << density << "\n";
    }
